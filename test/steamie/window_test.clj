...
(deftest own-game-test
  ""
  (is (= true (own-game? 10 [1 2 3 10]))))

(deftest match-game-test
  ""
  (is (= {:76561197960375033 {:time 72}} (match-game 10 (first example-database)))))

(deftest match-time-test
  ""
  (is (= {:76561197960375033 {:time 50}}
         (match-time {:appid 10, :playtime_forever 65, :poisson {:upper 80.80202518666516, :lower 49.197974813334845}} {:76561197960375033 {:time 50}}) )))
...
(deftest search-for-matching-games-test
  (let [game-list (map :appid example-user-profile)
        result (search-for-matching-games example-user-profile game-list example-database)]
    ""
    (is (= 2 (count result)))
    ""
...
(deftest collate-games-test
  (let [result (collate-games [{:10 [{:76561197960375033 {:time 72, :games [10 20 30 40 50 60 70 130 220 240]}} {:76561197969555013 {:time 79, :games [10 20 30 40 50 60 70 130 220 230]}}]} {:550 [{:76561197960667429 {:time 900, :games [550 20 30 40 50 60 70 130 80 100]}}]}])]
    (is (= result [10 20 30 40 50 60 70 130 220 240 10 20 30 40 50 60 70 130 220 230 550 20 30 40 50 60 70 130 80 100]))))
